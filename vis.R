...
plot_token_dist_UNSC <- function() {
  ...
  g <- ggplot(agg, aes(x = bin, y = value, fill = variable)) +
    ...
    geom_text(aes(x, y, label = lab),
      data = data.frame(
        x = Inf, y = Inf, lab = c(paste0("$\\Sigma$ [T] = ", formatC(sums[1, 2], format = "e", digits = 2)), paste0("$\\Sigma$ [S] = ", prettyNum(sums[2, 2], big.mark = ","))),
  ...
  g <- ggplot(agg, aes(x = bin, y = value, fill = variable)) +
    ...
    geom_text(aes(x, y, label = lab),
      data = data.frame(
        x = Inf, y = Inf, lab = c(paste0("$\\Sigma$ [T] = ", formatC(sums[1, 3], format = "e", digits = 2)), paste0("$\\Sigma$ [S] = ", prettyNum(sums[2, 3], big.mark = ",")), paste0("$\\Sigma$ [T] = ", formatC(sums[3, 3], format = "e", digits = 2)), paste0("$\\Sigma$ [S] = ", prettyNum(sums[4, 3], big.mark = ","))),
...
plot_token_dist_US <- function() {
  ...
  g <- ggplot(agg, aes(x = bin, y = value, fill = variable)) +
    ...
    geom_text(aes(x, y, label = lab),
      data = data.frame(
        x = Inf, y = Inf, lab = c(paste0("$\\Sigma$ = ", formatC(sums[1, 2], format = "e", digits = 2)), paste0("$\\Sigma$ = ", prettyNum(sums[2, 2], big.mark = ","))),
...
plot_model_evolution <- function(path) {
  ...
  levels(stats$type) <- c(
    "Learning Rate Profile", "Classification Accuracy",
    "Cross-Entropy Loss"
  )
  levels(stats$variable) <- c(
    "Training", "Validation",
    "Learning Rate"
  )
...
plot_token_dist_pred_UNSC <- function(path) {
  ...
  tikz("token_dist_pred_UNSC_length.tex",
    width = 20, height = 15, standAlone = TRUE,
  ...
  g <- ggplot(agg, aes(x = bin, y = value, fill = variable)) +
    ...
    geom_text(aes(x, y, label = lab),
      data = data.frame(
        x = c(0, Inf), y = c(Inf, Inf),
        lab = c(paste0("$\\Sigma$ [T] = ", formatC(sums[1, 2], format = "e", digits = 2), "\n$\\Sigma$ [N] = ", formatC(sums_cat[1, 3], format = "e", digits = 2), "\n$\\Sigma$ [C] = ", formatC(sums_cat[2, 3], format = "e", digits = 2), "\n$\\Sigma$ [P] = ", formatC(sums_cat[3, 3], format = "e", digits = 2)), paste0("$\\begin{aligned}", " \\Sigma ~[\\text{S}] &= \\text{", prettyNum(sums[2, 2], big.mark = ","), "} \\\\ \\Sigma ~[\\text{A}] &= \\text{", prettyNum(sums_cat[4, 3], big.mark = ","), "} \\\\ \\Sigma ~[\\text{O}] &= \\text{", prettyNum(sums_cat[5, 3], big.mark = ","), "} \\end{aligned}$")), type = sums[, 1], variable = unique(agg$variable)[1:2]
      ), hjust = c(-0.2, 1.2), vjust = c(1.2, 5), size = 10
    ) +
    xlab("\nBinned Speech Length [Tokens]") +
    ylab("") +
    theme_bw() +
